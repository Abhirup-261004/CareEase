...
function loadState() {
  ...
  } catch { state.reminders = []; state.missed = []; }
}
function saveState() {
...
function render() {
  ...
  const data = state.reminders.slice().sort((a,b)=> a.time.localeCompare(b.time))
  ...
  // missed
  missedList.innerHTML = state.missed.slice().reverse().map(m => `
...
function nextFireDate(rem) {
  ...
  if (rem.repeat === 'weekly') {
    ...
    if (!last) {
      ...
      const d = new Date(last); d.setDate(d.getDate()+7); d.setHours(hh,mm,0,0); return d;
    }
  }
  return base;
}
...
function startBell() {
  try {
    ...
    const stop = () => { clearInterval(iv); o.stop(); ctx.close(); };
    return stop;
  } catch { return () => {}; }
}
...
list?.addEventListener('click', (e) => {
  ...
  if (e.target.classList.contains('delete')) {
    ...
    r.enabled = e.target.checked; saveState(); render();
  }
});
...
missedList?.addEventListener('click', (e) => {
  ...
  if (e.target.classList.contains('renotify')) {
    ...
    if (idx >= 0) { state.missed.splice(idx,1); saveState(); render(); }
  }
});
